...
#    record.close()
    ...
    train_rec_name = 'trian_dataset.rec'
    trian_idx_name = 'train_dataset.idx'
    val_rec_name = 'val_dataset.rec'
    val_idx_name = 'val_dataset.idx'
    train_dataset = mx.recordio.MXIndexedRecordIO('train_dataset.idx', 'trian_dataset.rec','w')
    val_dataset = mx.recordio.MXIndexedRecordIO('val_dataset.idx','val_dataset.rec','w')
    trian_image_num = int(len(imglist)) * 0.8
    shuffle(imglist)
    for i in range(len(imglist)):
        imgname = imglist[i]
        img = cv2.imread(imgroot+imgname)
        ...
        header = mx.recordio.IRHeader(flag=0, label=bboxR.flatten(), id=0, id2=0)
        s = mx.recordio.pack_img(header, imgR, quality=100, img_fmt='.jpg')
        if i < trian_image_num:
            train_dataset.write_idx(i,s)
            print("Picture {} has been added into trainning dataset." .format(imgname))
        else:
            val_dataset.write_idx(i-trian_image_num,s)
            print("Picture {} has been added into validatoin dataset.".format(imgname))

    print("Threr are {} images have been added into training dataset".format(str(trian_image_num)))
    print("There are {} images have been added into validation dataset".format(str(len(imglist)-trian_image_num)))
    train_dataset.close()
    val_dataset.close()
